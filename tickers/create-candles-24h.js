...
const MIN_TIME_WINDOW_LEN_IN_SEC = 3600*24 // ==86400
...
const create_candles= async ( timeend , listassets ) =>{
	...
	let dblocal = db // dbstreams // dbtickers
	listassets.forEach ( async elem => { // LOGGER(elem.APISymbol)
		...
		let arrelemcandles=		await dblocal [ 'tickercandles' ].findAll ( {				raw : true  //			, attributes : [ 'close', 'volume' ]
			, where : { 
					timestamp : {
						[ Op.gte ] : timestart ,
						[ Op.lt ] :  timeend
		...
		if ( arrelemcandles && arrelemcandles.length ) {
			...
			await cliredisa.hset ('24H_TICKER_CLOSE' , symbol , jcandledata['close'] )
			await cliredisa.hset ('24H_TICKER_CLOSE_UPDATE' , symbol , timeend )
...
const main=_=>{
	cron.schedule( '0 0 0 * * *' , async _=>{ //	cron.schedule( '0-59/5 * * * * *' , async _=>{ //	cron.schedule( '* * * * * *' , async _=>{
