...
const MIN_TIME_WINDOW_LEN_IN_SEC = 5 // 6 0
...
//		let { APISymbol : symbol , id : assetId } = elem
		let { symbol , id : assetId } = elem // APISymbol : 
		let arrelemcandles=		await db[ 'tickercandles' ].findAll ( {				raw : true  //			, attributes : [ 'close', 'volume' ]
			, where : { 
					timestamp : {
						[ Op.gte ] : timestart ,
						[ Op.lt ] :  timeend
...
const main=_=>{
	cron.schedule( '*/5 * * * * *' , async _=>{ //	cron.schedule( '0-59/5 * * * * *' , async _=>{ //	cron.schedule( '* * * * * *' , async _=>{
