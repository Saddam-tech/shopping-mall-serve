...
router.get("/rows/betlogs/:offset/:limit", async (req, res) => {
  ...
  limit = +limit;
  db["betlogs"]
    .findAll({ raw: true, where: {}, offset, limit, order: [["id", "DESC"]] })
    .then(async (list) => {
      let aemails = [];
      list.forEach((elem) => {
      ...
      let countrows = await countrows_scalar("betlogs", {});
...
router.get("/status", async (req, res) => {
  ...
  let jrowcounts = await getrowcounts();
  let storageinfo = await osutils.drive.info();
	let socketcount = await cliredisa.get ( 'SOCKETCOUNT' )
  respok(res, null, null, { liststatus, jrowcounts, storageinfo , socketcount });
});
