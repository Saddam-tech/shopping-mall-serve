...
/**	jsymbol_ticker = await get_valid_tickers_with_min_timediff_from_reftime ( { 
		reftimepointsec : timerefsec 
...
//	jsymbol_ticker = await get_valid_tickers_with_orderterm ({		reftimepointsec : timerefsec 		, timedeltarequiredsec:10		, orderterm : 'lte'		})
	...
	LOGGER( `@jsymbol_ticker-openbets` , jsymbol_ticker , timerefsec  ) 
//	let listbets =await fi ndall ( 'bets' , { starting : timerefsec }  )
	...
	listbets.forEach ( async elem => {
		...
		else { LOGGER( `@!!!bet has no ticker `) ; return }
//		await u pdaterow ( 'bets' , { id } , { startingPrice : ticker_price , entryprice1 : ticker_price } )
		await db[ 'bets'].update ( { startingPrice : ticker_price , entryprice1 : ticker_price } , { where : {id} } , {transaction } ) 
	})
	transaction.commit()
}
