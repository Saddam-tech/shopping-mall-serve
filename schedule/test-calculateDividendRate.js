...
/** const get_tic kers_conforms_to_gettickerprice_source = async ( {})=>{
	let list_assets = await findall("assets", { active: 1 , APISymbol : { [ Op.ne ] : null } });
  list_assets.forEach(async (elem) => {
	...
	Object.keys ( jexpiry_arrbets ).forEach ( async key => { 
		...
		jexpiry_arrbets [ key ].forEach ( async bet => { let { id } =bet
			let tellresult = tell_win_lose ( bet ) ; LOGGER( `@tellresult` , tellresult )
				switch ( tellresult  ){				
				case 1 : betsumamounts ['win'] += +bet.amount ;jbetid_winlose [id] = 1;  break // ;await db['bets'].update ( {status : 1 , } , {where:{ id: bet.id } }) ;
				case -1: betsumamounts ['lose']+= +bet.amount ;jbetid_winlose [id] = 0;break   // ;await db['bets'].update ( {status : 0 , } , {where:{ id: bet.id } }) ;
				case 0 : jbetid_winlose [id] = 2;break //await  db['bets'].update ( {status : 2 , } , {where:{ id: bet.id } }) ;
				default : break
			}
		}) // end rach expiry
	})
		...
		if ( betsumamounts['win'] == 0 ||betsumamounts['lose'] == 0 ){
			...
			jexpiry_arrbets [ expiry ].forEach ( async bet => { let { id} = bet
				switch ( jbetid_winlose [ id ] ) {
					case 1 : await db['bets'].update ( { status : 1 , diffRate  : (diffRate * 100).toFixed(2) ,winamount : diffRate * +bet.amount } , {where:{ id } }) ; break // : (diffRate * 100).toFixed(6)
