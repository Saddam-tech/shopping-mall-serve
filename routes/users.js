...
async function generateRefCode(i = 0) {
  let code = String(crypto.createHash("md5").update( ''+moment().unix() ).digest("hex")).slice(
    i,
    i + 14 // 10
  );
  console.log(code);
...
async function createJWT( { jfilter , userinfo } ) {
  let userwallet;
  if ( userinfo ) {}
	else { userinfo = await db["users"].findOne({
    where: {      ...jfilter,
    },
    attributes: [
      "id",
...
/*  await db["userwallets"]    .findOne({
	...
	let mywallet = await db['userwallets'].findOne( { raw: true , where : { uid : userinfo?.id } } )
  if (!userinfo) {
    return false;
  }
	let expiresIn = "48h"
...
router.post ( '/login' , async ( req,res)=>{
	...
	let respuser = await db[ 'users' ].findOne ({ raw: true , where : { email, password , nettype } } )
	if ( respuser ) {}
...
router.post ( '/verify/emailcode/:email/:code' , async ( req,res)=>{
	...
	let resp = await db['emailverifycodes'].findOne ( { raw: true, 	
		where : { email, active: 1 } } 
	)
	if ( resp ) {}
	else { resperr ( res, messages.MSG_DATANOTFOUND ) ; return }
	if ( resp.code == code ) {}
	else { resperr ( res, messages.MSG_DATAINVALID ) ; return }
	respok ( res )  
})
router.post ( '/signup' , async ( req,res)=>{
	...
	else { resperr ( res, messags.MSG_ARGINVALID ) ; return }
	let respemail = await db['users'].findOne ( { raw : true , where : { email , nettype } } )
	if ( respemail ) {	resperr( res, messages.MSG_DUPLICATEDATA ) ; return }	else {}
	const myreferercode= await generateRefCode()
	let user = await db[ 'users'].create ( {email , password , myreferercode , nettype } )
...
router.get( '/email/code' , async ( req,res)=>{
	...
	let respexpires = await db['settings'].findOne ( {raw: true
		, where : {	
