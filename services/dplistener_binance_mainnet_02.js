...
let wss_rpcURL = 'wss://ws-nd-758-768-804.p2pify.com/fdeab5b459568645655ce1e15e3f059d' // wss://ws-nd-705-872-619.p2pify.com/603af1787e985df8fa39dbc3cf659d46' // wss_rpcURLs [ nettype ]	BSC_MAINNET :  , // 
const INST_ID = 2
...
const watchTokenTransfers = async () => {
  ...
  tokenContract.events.Transfer(options, async (err, ev) => {
    ...
    ( counttx++ % PERIOD_COUNT_DISP == 0 ) && console.log(`Deposit from ${_from} to ${_to} amount ${getethrep (_value ) }-${nettype},${ev?.blockNumber},${counttx },${INST_ID}`);
false &&	LOGGER ( ev )
		let status
		let { removed } =ev
		if ( removed ) { status = 1 }
