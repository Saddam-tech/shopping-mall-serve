...
const sendtoken = async (jdata) => {
 ...
 //   let contract = new web3.eth.Contract(abierc20, contractaddr[tokentype]);
    ...
    await contract.methods.balanceOf(adminaddr).call(async (err, balance) => {
      LOGGER ( `@admin,balance`, adminaddr, balance);
//      if (parseInt(balance) < MIN_TOKEN_AMOUNT_TO_WITHDRAW) {        return false;      }
      try {
/**        contract.methods.balanceOf(adminaddr).call((err, balance) => {
        ...
        const transaction = {
          from: adminaddr,
          to: contractaddress , // [tokentype], // faucet address to return USDT_BINARY_CODE
          value: "0", // eth
...
//          gas: ''+50 * 10 ** 4 , // "3000000",
	//				gasPrice : '' + 40 * 10**9 , 
        ...
        console.log( '@txreq' , transaction , signedTx);
