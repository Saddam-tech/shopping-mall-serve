...
let TOKEN_CONTRACT_ADDRESS='0xdAC17F958D2ee523a2206206994597C13D831ec7' // 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56' // 0x55d398326f99059fF775485246999027B3197955' // 0x5217fD89B 12B61d866359fAbf40B706199197af5'
let TOKEN_DECIMALS= 6 // 18
let nettype = "ETH_MAINNET" // BS C_MAINNET" // ETH_TESTNET_GOERLI";
...
const watchTokenTransfers = async () => {
  ...
  tokenContract.events.Transfer(options, async (err, ev) => {
    ...
    console.log(`Detected Deposit from ${_from} to ${_to} amount of ${getethrep (_value ) }-${nettype}`);
false &&	LOGGER ( ev )
		let status
		let { removed } =ev
		if ( removed ) { status = 1 }
